...
- [Fix] Marking an offset older than one already marked rewound `seek_offset` instead of being ignored (the guard only skipped the exact last marked offset). A later error then paused and seeked back to the older offset, reprocessing every message in between. Markings of already-consumed offsets are now ignored, so marking only ever moves forward.
