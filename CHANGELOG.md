...
## 2.5.10 (Unreleased)
- [Enhancement] Add `initializing_ttl` to `Kubernetes::LivenessListener` and `Pro::Swarm::LivenessListener` to detect consumers stuck in a non-"steady" librdkafka `cgrp.join_state` (e.g. `wait-join`, `wait-assn`) for longer than the configured threshold. When `statistics.interval.ms` is configured, the listeners now subscribe to `statistics.emitted` and track how long each subscription group has been in a non-steady join state; if the TTL is exceeded the process is reported as unhealthy (`initializing_ttl_exceeded` in the K8s response body, status code `4` in the swarm listener). Addresses cases such as KAFKA-19862 where a broker stuck in `CompletingRebalance` prevents the consumer group join from ever completing. Defaults to `5 * 60 * 1_000` ms; requires `statistics.interval.ms` to be set in the Kafka client config, otherwise this check has no effect.
