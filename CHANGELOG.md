...
- [Enhancement] Introduce the share group (KIP-932 / Queues for Kafka) routing layer: `share_group` routing blocks with validation, introspection (`#group_type`, `App.share_groups`, `App.routes.consumer_groups`/`App.routes.share_groups`) and `--include_share_groups`/`--exclude_share_groups` CLI filters. Share groups can be described but not yet run - the server raises `Karafka::Errors::ShareGroupsNotImplementedError` until the runtime lands. Internally, routing is reorganized into per-mode `Routing::{ConsumerGroups,ShareGroups}` namespaces with backwards-compatible aliases. No behavior change for consumer-group routing.
- [Enhancement] Make the client-configuration seams share-group aware: `Setup::AttributesMap` and `Setup::DefaultsInjector` gain `.consumer_group`/`.share_group` scopes (`.consumer` remains a legacy alias), and subscription groups built for share groups skip consumer-group-only client settings (`auto.offset.reset`, `enable.auto.offset.store`, `max.poll.interval.ms`, `group.instance.id`) that librdkafka rejects for KIP-932 share consumers. Share topics no longer validate `initial_offset` (share groups use the broker-side `share.auto.offset.reset`).
