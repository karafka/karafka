...
- [Fix] Stop `JobsQueue#tick` from accumulating unbounded, unconsumed signals on the per-group semaphore `Queue`. The semaphore is only ever used as a level-triggered wakeup, but `#wait` pops at most one entry per loop iteration and, for pure non-blocking/LRJ jobs, never enters its loop at all (`#wait?` short-circuits to `false`), so every `#complete` and every non-blocking job start (`Worker`) pushed a signal that was never drained - unbounded heap growth proportional to the number of non-blocking jobs processed, over the lifetime of the process. `#tick` now drains any stale signals before pushing a new one, keeping the semaphore bounded regardless of whether `#wait` is actively consuming it.
