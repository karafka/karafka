...
## 2.5.10 (Unreleased)
- [Enhancement] Add `rebalance_ttl` to `Kubernetes::LivenessListener` and `Pro::Swarm::LivenessListener` to detect consumers permanently stuck in a non-`"steady"` librdkafka `cgrp.join_state`. The librdkafka `join_state` field tracks the consumer group join protocol: `"steady"` means the consumer has successfully joined the group and is operating normally; any other value (`wait-join`, `wait-assn`, `wait-sync`, `wait-unassign-to-complete`, etc.) means a rebalance or initial join is in progress. Brief non-steady periods are expected during normal rebalances — the `rebalance_ttl` only triggers when a group stays non-steady continuously for longer than the threshold, which indicates the consumer is permanently stuck (e.g. due to broker-side bugs like KAFKA-19862 where the broker hangs in `CompletingRebalance` and never responds to the consumer's JoinGroup/SyncGroup requests). When exceeded, `rebalance_ttl_exceeded` is set to `true` in the `Kubernetes::LivenessListener` HTTP response body (HTTP 500) and status code `4` is reported by `Pro::Swarm::LivenessListener`. Recovery is automatic: as soon as `join_state` returns to `"steady"`, the flag clears and the probe returns healthy. **Requires `statistics.interval.ms` to be set** in the Kafka client config — without it `statistics.emitted` never fires and this check has no effect (no false positives). **Set `rebalance_ttl` to at least your `max.poll.interval.ms`** to avoid false positives during slow but legitimate cooperative rebalances where the `wait-unassign-to-complete` phase can approach `max.poll.interval.ms`. Defaults to `10 * 60 * 1_000` ms (10 minutes), which provides headroom above the Kafka default `max.poll.interval.ms` of 5 minutes.
