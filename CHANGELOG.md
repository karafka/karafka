...
## 2.5.10 (Unreleased)
- [Enhancement] Add `rebalance_ttl` to `Kubernetes::LivenessListener` and `Pro::Swarm::LivenessListener` to detect consumers stuck in a non-"steady" librdkafka `cgrp.join_state` (e.g. `wait-join`, `wait-assn`, `wait-sync`) for longer than the configured threshold. This covers both the initial consumer group join and subsequent rebalances. When `statistics.interval.ms` is configured, the listeners subscribe to `statistics.emitted` and track how long each subscription group has been in a non-steady join state; if the TTL is exceeded the process is reported as unhealthy (`rebalance_ttl_exceeded` in the K8s response body, status code `4` in the swarm listener). Addresses cases such as KAFKA-19862 where a broker stuck in `CompletingRebalance` prevents the consumer group join from ever completing. Defaults to `10 * 60 * 1_000` ms — set this to at least your `max.poll.interval.ms` to avoid false positives during slow but legitimate rebalances. Requires `statistics.interval.ms` to be set in the Kafka client config, otherwise this check has no effect.
