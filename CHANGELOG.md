...
- **[Feature]** Allow declarative topic migrations to target a custom Kafka cluster via `Karafka::Cli::Topics::Migrate.new(kafka: config).call`, without mutating the application-wide Kafka configuration. The existing no-argument migration continues to use the default cluster.
