...
## 2.5.10 (Unreleased)
- [Enhancement] Namespace consumer-group-specific Pro admin utilities under `Karafka::Pro::Admin::ConsumerGroups` (internal); `Karafka::Admin::Recovery` alias is unchanged and remains the public entry point. Prepares the Pro admin namespace for parallel `ShareGroups` utilities (Pro).
