...
## 2.6.0 (Unreleased)
- [Enhancement] Refresh watermark offsets and lags of long-paused partitions in `statistics.emitted` (Pro). librdkafka only updates those values from fetch responses, so long-paused partitions report frozen values. Enabled via `config.internal.statistics.consumer_groups.lag_compensation.interval` (ms, default `0` - disabled); partitions qualify after staying paused for at least `lag_compensation.pause_age` (ms, min `5_000`, default `30_000`). Requires `statistics.interval.ms` to be enabled. On resume the refreshed values are kept (refreshing stops) so the compensator hands over to the live statistics only once librdkafka post-resume fetches catch up, instead of the emitted values snapping back to the frozen pre-resume ones in the meantime; kept values are dropped in bulk on the next rebalance. Known edge case: the refresh uses the batched `ListOffsets` API, which resolves the end offset to the high watermark regardless of the consumer isolation level, so while a transaction is in flight on a paused partition the compensated lag can transiently overstate the `read_committed` lag by the number of uncommitted messages (it self-corrects once the transaction resolves; non-transactional topics are unaffected).
- **[Feature]** Add `Karafka::Connection::Client#read_partition_offsets` (and `Connection::Proxy#read_partition_offsets`) exposing the batched `ListOffsets` query on a consumer own connection. Takes the same partition offset specs and returns the same resolved offsets as `Karafka::Admin.read_partition_offsets`, with the same retry and timeout recovery as `#query_watermark_offsets` (it reuses its `config.internal.connection.proxy.query_watermark_offsets` settings, being the same broker query, only batched). The client-level API forwards the consumer own `isolation.level` to the query. Note that the `ListOffsets` admin API resolves `:latest` to the high watermark regardless of the isolation level (unlike the per-partition `#query_watermark_offsets`, which returns the Last Stable Offset for a `read_committed` consumer), so on a topic with an in-flight transaction `:latest` includes the uncommitted messages; non-transactional topics are unaffected.
- [Change] Require `karafka-rdkafka` `>= 0.28.0`, which exposes `Rdkafka::Consumer#list_offsets` and rebuilds `Rdkafka::Consumer#lag` on top of it. Lag reads (`Karafka::Admin.read_lags_with_offsets` and everything built on it) now issue a single batched end-offsets query instead of one blocking watermark roundtrip per partition.
- **[Feature]** Allow `Karafka::Admin` (and its subsystems like `Admin::ConsumerGroups` and `Admin::Topics`) to operate on an external client: `Karafka::Admin.new(external_client: client)`. Accepted forms: a raw rdkafka client, one wrapped with `Karafka::Connection::Proxy` or a running consumer `Karafka::Connection::Client` (which now exposes its wrapping proxy via `#wrapped_kafka`), so a consumer can for example read its own lags through its own connection from within `#consume`. Connection clients are re-resolved on each use, so the admin instance follows them across their underlying connection recovery resets; raw and proxied instances are resolved once at construction. Routing is capability based: when the external client is an rdkafka admin instance, admin-based operations (`with_admin` and everything built on it, e.g. `cluster_info`) run on it; any other external client is used by consumer-based operations (`with_consumer` and everything built on it, e.g. `read_lags_with_offsets`). Either way the external instance replaces the short-lived dedicated one, saving a full client bootstrap per call. The external client lifecycle stays fully with its owner: it is never configured, started, unsubscribed or closed by the admin APIs. Low-level internal API: operations run within the external client identity (including its `group.id`) and the caller is responsible for invoking only operations that are safe on a live client (e.g. lag/offset reads for its own group).
- [Change] Remove the public `custom_kafka` reader from `Karafka::Admin`. It was an internal collaboration detail exposed by the multi-cluster support introduction and is not used by any public API - custom kafka settings are provided at construction time (`Karafka::Admin.new(kafka: ...)`) and never need to be read back.
- [Enhancement] Make the statistics decorator used by `Instrumentation::Callbacks::ConsumerGroups::Statistics` configurable via `config.internal.statistics.consumer_groups.decorator_class`, instead of hardcoding `Karafka::Core::Monitoring::StatisticsDecorator`. The default is a new `Instrumentation::Callbacks::ConsumerGroups::Decorator` subclass (currently a thin pass-through), giving this callback a swappable extension point for a future decorator without needing to change the callback itself.
...
- [Enhancement] Declare recurring tasks (`schedules`, `logs`) and scheduled messages (`messages`, `states`) topics via the standalone `Karafka::App.declaratives.draw` DSL instead of the routing `config(...)` bridge, so these internal Pro features no longer depend on the to-be-retired bridge. The resulting declarations (partitions, replication factor, and Kafka config) are unchanged, and yield-style reconfiguration (mutating `topic.config` on the shared declaration) keeps working since routing and declaratives share the same declaration object (Pro).
...
- [Enhancement] Add `Pro::Processing::ConsumerGroups::Filters::Actions` as the single source of truth for filter post-execution actions: `Actions.skip` / `Actions.pause` / `Actions.seek` value methods (so filters can return `Actions.pause` instead of the raw `:pause` symbol), the `ALL` list derived from them, and `#skip?` / `#pause?` / `#seek?` predicate helpers built on `#action`. The predicates are mixed into both `Filters::Base` and the aggregating `Coordinators::FiltersApplier`, and all internal filters and call sites now use `Actions.*` / `filter.pause?` instead of raw symbols. The returned values are the same plain symbols, so filters returning `:skip`/`:pause`/`:seek` directly keep working - provided they inherit `Filters::Base` (already the documented requirement for all filters), which is what supplies the predicate helpers the applier now relies on (Pro).
- [Enhancement] Replace per-partition `query_watermark_offsets` consumer calls in `Admin::Topics#read_watermark_offsets` with two batch `list_offsets` admin calls (`:earliest` and `:latest`), reducing N sequential consumer roundtrips to 2 admin roundtrips regardless of how many topics or partitions are queried. Replace per-partition `query_watermark_offsets` fallback in `Admin::ConsumerGroups#seek` (when a time-based offset resolves beyond the last message) with a single batch `list_offsets` call covering all affected partitions. Replace per-partition `query_watermark_offsets` loop in `Pro::Iterator::TplBuilder#resolve_partitions_with_negative_offsets` with one batched `offsets_for_times(epoch)` call for consumer metadata warm-up plus two batched `list_offsets` calls for LWM/HWM resolution, reducing N sequential calls to 3 total. All three run on the iterator own consumer connection, so resolving negative offsets no longer creates a dedicated admin client (Pro).
...
- [Fix] Recurring tasks compared the stored schedule version against the current one as plain strings, so multi-segment versions ordered lexicographically: a valid upgrade such as `1.9.0` to `1.10.0` was read as a downgrade (`"1.9.0" > "1.10.0"`) and rejected as incompatible, raising `IncompatibleScheduleError` and blocking all task execution. Versions are now compared with `Gem::Version`, matching the scheduled messages schema-version check (Pro).
- [Fix] Dynamic multiplexing `scale_delay` was applied globally instead of per subscription-group family. `Pro::Connection::Manager#stable?` memoized the value with `@scale_delay ||=` on the single process-wide manager, so whichever family was evaluated first permanently fixed the stability window for every other family - making each consumer group with a distinct `scale_delay` ignore its own setting (premature or sluggish scaling). The window is now read as a local per family on every check (Pro).
- [Fix] `Routing::Topics#<<` appended to its backing array in place, so a topic discovered at runtime by a pattern subscription (on a listener thread) could be observed mid-append by a concurrent reader iterating the same routing topics - for example `Router#find_by` from a `Pro::Iterator` or `Admin` call on another thread. Appends are now copy-on-write (a new array is published atomically), so concurrent iteration always traverses a complete snapshot on every Ruby runtime, not only under MRI's GIL.
- [Fix] `Instrumentation::AssignmentsTracker` never reduced its `@generations` map (generations are kept after revoke so they can keep incrementing on reassignment), so under regex pattern subscriptions - which inject a fresh, permanent routing topic per discovered name - the map and the frozen deep copy built by `#generations` grew for the whole process lifetime. The map is now bounded (`GENERATIONS_TOPICS_LIMIT`), evicting the least-recently-assigned topics once exceeded; static topologies never reach the bound, so their behaviour is unchanged.
- [Fix] Filtering `FiltersApplier#timeout` took the minimum across all applied filters, so when the resolved action was `:pause`, a coexisting filter that resolved to `:seek` (and reports a `0` timeout instead of the contract's `nil`) collapsed the pause to `0`. The partition then paused with timeout `0`, expired immediately and busy-spun instead of backing off. The pause timeout is now derived only from the filters that actually want to pause, keeping the documented minimum-of-pausing-timeouts semantics, so a seek filter's `0` no longer defeats the backoff (Pro).
- [Fix] Scheduled messages could flip from loading to loaded and dispatch before the whole stream was read when EOF never fires (high turnover) and a schedule carried a CreateTime in the future (producer clock skew or an explicitly set timestamp). The loaded heuristic compared the last message's CreateTime against `started_at + grace`, so a single future-dated message read mid-backlog satisfied it while older cancel tombstones at higher offsets were still unread - dispatching schedules whose cancellations had not been loaded yet (double dispatch on recovery/replay). The CreateTime is now clamped to the wall clock (`[timestamp, now].min`), mirroring the batch metadata builder, so a future timestamp can no longer be taken as proof of catch-up (Pro).
...
- [Fix] The `parallel_segments` CLI commands (`distribute`, `collapse`, `reset`) silently skipped every other consumer group when more than one was requested via `--groups`: `applicable_groups` deleted each matched group from the very array it was iterating with `each`, so the internal cursor jumped past the next entry. Requesting two groups processed only the first; the rest were left untouched without any error. Matched groups are no longer removed mid-iteration, so all requested groups are processed (Pro).
- [Fix] `Pro::Instrumentation::PerformanceTracker` accumulated a processing-time samples array for every `(topic, partition)` ever consumed and never released it on revoke or rebalance, so the map grew for the whole process lifetime - unbounded under regex pattern subscriptions that keep discovering new topic names. It now evicts a partition's samples when the partition is revoked (dropping the topic entry once it has no tracked partitions left), mirroring the offset metadata fetcher. Samples are also now scoped by subscription group id instead of `(topic, partition)` alone, so two consumer groups consuming the same topic keep independent measurements and revoking a partition in one group no longer drops samples another group is still using. **Breaking (internal API):** `PerformanceTracker#processing_time_p95` now takes the subscription group id as its first argument (`processing_time_p95(group_id, topic, partition)`); custom schedulers calling it must pass `job.group_id` (Pro).
...
- [Fix] The per-partition retry attempt counter was not reset on revocation: the pause tracker lives in the listener's pauses manager keyed by topic-partition and was reused as-is when the same listener reclaimed the partition. A message that had climbed toward its retry limit before a rebalance was treated as already that many attempts in afterwards - with DLQ, the next failure was dispatched immediately, skipping the configured retries. The attempt counter is now reset when a partition is revoked.
- [Fix] ActiveJob `CurrentAttributes` loaded from a job's payload were not reset when the job raised, leaking the failed job's attributes (e.g. tenant/user context) into the next job processed on the same worker that carried none of its own. The reset now runs in an `ensure`, so it happens on the error path too.
- [Fix] `#mark_as_consumed`, `#mark_as_consumed!` and `#commit_offsets!` could return `true` when the operation failed because the partition was lost. A failed marking or commit now always reports `false`.
- [Fix] A user `#seek` that raised (for example an unresolvable time-based offset) nilled `seek_offset` before the seek, so the failure-driven retry paused without seeking back and skipped the rest of the batch. `seek_offset` is now reset only after the seek succeeds.
- [Fix] ActiveJob + DLQ + long-running job strategies dispatched a failed message to the DLQ even when the partition had been revoked mid-job, producing a DLQ entry on a lost partition that the new owner then duplicated. They now skip the dispatch when revoked (matching the non-AJ LRJ DLQ strategies) and let the new owner handle it (Pro).
- [Fix] Transactional DLQ dispatch with `mark_after_dispatch: false` advanced the in-memory `seek_offset` inside the dispatch transaction; if that transaction aborted, the DLQ message was rolled back but the advanced offset survived, skipping the broken message (neither dispatched nor reprocessed). The offset is now advanced only after the dispatch transaction succeeds (Pro).
- [Fix] Virtual Partitions `#mark_as_consumed` and `#mark_as_consumed!` returned an inverted ownership result when there was no real offset to materialize yet (out of order marking before the virtual partition holding the lowest offset of a partition start marked anything): `false` (lost) on owned assignments and `true` after an actual loss. Ownership is now reported truthfully (Pro).
- [Fix] Recurring tasks commands consumed during the boot replay phase (dispatched while the process was down) were applied and then reverted by the end-of-replay schedule synchronization, with the reverted state persisted - losing the command permanently (Pro).
- [Fix] Virtual Partitions with DLQ could lose messages and dispatch a wrong one when the DLQ strategy resolved to `:dispatch` or `:skip` on a non-collapsed (parallel) run, e.g. a custom strategy dispatching on the first failure: the decision was based on a single virtual partition subset, DLQ-ing an arbitrary message from it and committing offsets of messages other virtual partitions never processed. Dispatch and skip decisions are now always preceded by a collapsed retry, so they happen on the linear, deterministic flow (Pro).
- [Fix] Scheduled messages schema versions were compared as strings, which would mis-evaluate compatibility once any version segment reaches two digits ("1.2.0" is older than "1.10.0" but lexicographically greater). Versions are now compared as `Gem::Version` (Pro).
- [Fix] `Karafka::Admin.read_topic` with a `Time` start offset beyond the last message returned the newest messages (all older than requested) instead of none: the "not found" time-resolution result was reinterpreted as the relative negative-offset form.
- [Fix] Prevent silent worker thread death that could drain the workers pool and stall all processing. The worker loop treated the processing flow result as its continue/stop signal, so a user `Consumer#wrap` returning a falsy value or an error reaching the critical rescue path killed the worker thread without any notification. Worker continuation is now an explicit framework decision: only a closed queue or a downscale sentinel stops a worker.
- [Fix] `Pro::Iterator` could silently skip messages in multi-partition iteration: EOF events were counted globally instead of per partition, while librdkafka re-emits EOF for the same partition each time it reaches the end again after new data arrived. A single partition receiving new data mid-iteration (or an explicitly stopped partition that also EOF-ed) could exhaust the EOF budget for the whole subscription and terminate `#each` while other partitions still had unread backlog. EOF and explicit stops are now tracked per topic-partition identity. Additionally, `#each` now resets the `#stop` flag when it finishes, so an iterator instance can be reused after an explicit stop, as documented (Pro).
...
- [Maintenance] Split declarative topic configuration from routing in the integration suite. Add a `draw_topics` spec helper (wrapping `Karafka::App.declaratives.draw`) that declares topic structure independently and eagerly creates the topics, source spec topic creation from the shared declaratives repository (`Karafka::App.declaratives.repository`) rather than walking the routing tree, and migrate the general integration specs to declare partitions/config via `draw_topics` instead of `config(...)` inside `draw_routes`. Specs that intentionally exercise the routing `config()` bridge (`cli/declaratives`, `routing/features/declaratives`, the routing `defaults` specs) keep using it for backwards-compat coverage.
