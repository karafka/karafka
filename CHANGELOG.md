...
## 2.6.0 (Unreleased)
- **[Feature]** Allow `Karafka::Admin` (and its subsystems like `Admin::ConsumerGroups` and `Admin::Topics`) to operate on a borrowed client: `Karafka::Admin.new(borrowed_client: consumer)`. When provided, consumer-based operations (`with_consumer` and everything built on it, e.g. `read_lags_with_offsets`) run on the borrowed instance instead of creating short-lived dedicated consumers, saving a full consumer bootstrap per call. The borrowed client lifecycle stays fully with its owner: it is never configured, started, unsubscribed or closed by the admin APIs. Low-level internal API: operations run within the borrowed client identity (including its `group.id`) and the caller is responsible for invoking only operations that are safe on a live consumer (e.g. lag/offset reads for its own group).
