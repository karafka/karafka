...
- [Enhancement] Introduce the share group (KIP-932 / Queues for Kafka) routing layer: a `share_group` routing block (sibling of `consumer_group`), mode namespaces `Routing::ConsumerGroups::{Group,Topic,Contracts}` and `Routing::ShareGroups::{Group,Topic,Contracts}` (matching the existing `consumer_groups`/`share_groups` organization, with each mode's validation contracts co-located under it) that keep share- and consumer-group feature flow separate, `#group_type`/`#consumer_group?`/`#share_group?` introspection, `App.share_groups`, and `--include_share_groups`/`--exclude_share_groups` CLI filters (also in swarm mode). The pre-existing flat `Routing::ConsumerGroup`/`Topic` constants are kept as aliases (retired in 3.0); share groups are only exposed under the new `Routing::ShareGroups::*` namespace. Describing share groups is supported; running one raises `Karafka::Errors::ShareGroupsNotImplementedError` until the runtime lands. Every routing feature is namespaced by exactly one mode - the previously shared OSS features (`deserializers`, `declaratives`) and the remaining top-level Pro features (filtering, throttling, expiring, pausing) all now live under `Features::ConsumerGroups::`, so a feature only declares kind-only `Topic`/`Group` hooks and its mode is taken from its namespace (there are no "shared" features - features that both modes need are duplicated under `Features::ShareGroups::`). Share topics currently mirror the consumer-group `deserializers` and `pause` features (`Features::ShareGroups::{Deserializers,Pausing}`), so they expose them in the same format; the per-topic `#pause` (backoff) config lives on each mode's topic class (`Routing::ConsumerGroups::Topic` / `Routing::ShareGroups::Topic`) rather than the shared base. The routing builder expander moved from `Features::Base::Expander` to `Features::Expander`. No behavior change for consumer-group routing.
