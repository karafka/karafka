...
## 2.6.2 (Unreleased)
- **[Feature]** Add `Instrumentation::Vendors::NewRelic::MetricsListener` for publishing Karafka metrics (rdkafka statistics, consumer, worker, and error events) to New Relic via `NewRelic::Agent.record_metric`. Since New Relic custom metrics do not support tags, context (consumer group, topic, partition, broker) is encoded directly in the metric name (svyatmuzyka).
- **[Feature]** Add `karafka info --extended` printing routing (consumer groups, subscription groups, topics, and their active features such as `dead_letter_queue`, `virtual_partitions`, `parallel_segments`, `multiplexing`, etc.), global app config, and the effective Kafka config. Sensitive Kafka config values (matching `password`, `secret`, `token`, or `sasl.oauthbearer.client`) are redacted as `[REDACTED]` in both the global section and per-topic `kafka` overrides.
- **[Feature]** Add `Kubernetes::ReadinessListener` for serving a Kubernetes readiness probe on its own HTTP port. It reports healthy once every active subscription group has started polling and not-ready again once the process is shutting down or quieting, so Kubernetes can drain a pod from the Service endpoints before it exits. Point a `startupProbe`/`readinessProbe` at it and a `livenessProbe` at `LivenessListener`.
...
- [Enhancement] Introduce the share group (KIP-932 / Queues for Kafka) routing layer as the first step of share group support. Adds a `share_group "name" do ... end` routing block (sibling of `consumer_group`), a `Routing::Groups::{Base,ConsumerGroup,ShareGroup}` group hierarchy and a `Routing::Topics::{Base,ConsumerTopic,ShareTopic}` topic hierarchy so share-group feature flow stays independent of consumer-group feature flow, `#group_type` / `#consumer_group?` / `#share_group?` introspection, `Karafka::App.share_groups`, and `--include_share_groups` / `--exclude_share_groups` CLI server filters (also available in swarm mode). The flat `Routing::ConsumerGroup` and `Routing::Topic` constants are preserved as aliases. Share groups can be described in the routing today, but running them is not yet implemented: the server raises `Karafka::Errors::ShareGroupsNotImplementedError` when it detects a share group at boot. Internal-only preparation for KIP-932; no behavior change for existing consumer-group routing.
...
- [Fix] [Pro] Stabilize the `Karafka::Admin::Recovery` `read_committed_offsets` no-offsets integration spec against a fresh CI broker where the `__consumer_offsets` internal topic does not yet exist. The spec deliberately never commits for its own group, so when it ran early in the suite (before any other group had committed) `offsets_partition_count` raised `MetadataError: Could not retrieve partition count for '__consumer_offsets'`. It now commits an offset for a separate throwaway group to force the internal topic into existence and retries the recovery call with exponential backoff for up to ~30s, mirroring the already-hardened `offsets_partition_for`/`coordinator_for` specs. The asserted result for the tested group stays empty.
