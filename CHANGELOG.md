...
- [Fix] A forceful supervised shutdown (a job exceeding `shutdown_timeout`) called `Kernel.exit!` directly, which skips the `ensure` block that flushes and closes the producer. Async-buffered messages - DLQ `produce_async` copies and user `produce_async` calls - were then discarded even though the consumer offsets had already been committed during the listener shutdown, causing silent message loss. The producer (and the WaterDrop connection pool) are now flushed and closed (time-boxed by `forceful_shutdown_wait`) before the forceful `exit!`.
