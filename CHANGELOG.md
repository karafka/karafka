...
## 2.6.1 (Unreleased)
- **[Feature]** Add a fibers workers execution backend (`config.workers.backend = :fibers`, requires adding the `async` gem to the application bundle). Workers run as fibers hosted on `config.workers.carrier_threads` carrier threads (default `1`), each running an `Async` reactor, aimed at IO-bound workloads where high `workers.concurrency` values should not cost a thread each. The job distribution model, `concurrency` semantics, jobs queue statistics, downscale sentinel protocol and all `worker.*` instrumentation events are identical to the threads backend, and errors escaping the worker processing loop crash the process loudly (parity with the threads workers `abort_on_exception` behavior); the threads backend remains the default. When using Rails/ActiveSupport, set `config.active_support.isolation_level = :fiber` so execution state (e.g. `CurrentAttributes`) does not leak between jobs sharing a carrier thread. Note that C-level calls that do not yield to the fiber scheduler (e.g. librdkafka sync operations, some DB drivers) block their whole carrier thread - increase `workers.carrier_threads` for workloads mixing such calls with latency-sensitive jobs.
- [Enhancement] Introduce the `config.workers` namespace (`workers.backend`, `workers.concurrency`, `workers.thread_priority`, `workers.carrier_threads`) as the canonical home for worker execution settings, preparing the seam for the upcoming fibers execution backend. `config.concurrency` and `config.worker_thread_priority` remain available as root-level aliases that forward to the nested settings; their writers emit a one-time deprecation warning and both aliases will be removed in a future major release. `config.to_h` no longer contains the root-level `concurrency` and `worker_thread_priority` keys (values live under `workers`).
- [Enhancement] Make the workers pool class configurable via `config.internal.processing.workers_pool_class` (defaults to `Processing::WorkersPool`), the seam via which alternative execution backends will plug in their pool implementations.
