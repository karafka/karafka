...
## 2.6.0 (Unreleased)
- **[Feature]** Add active refreshing of watermarks and lags for long-paused partitions (Pro). librdkafka only updates watermark offsets and lags from fetch responses, so partitions paused for a long time (scheduled messages, delayed topics, long pauses from filtering or DLQ backoffs) report frozen values in `statistics.emitted`. When enabled via `config.internal.statistics.paused_refresh.interval` (ms, default `0` - disabled), a Pro refresher runs on the listener threads (via `client.events_poll`), tracks pause state through the `client.pause` / `client.resume` instrumentation events, refreshes partitions that stayed paused for at least one full interval and fetches their committed offsets (one batched query) and watermarks through the running consumer connection - no dedicated instances are created. A Pro statistics decorator overlays the refreshed `lo_offset`, `hi_offset`, `committed_offset`, `consumer_lag` and `consumer_lag_stored` onto the raw statistics before standard decoration, touching nothing when no refreshed data exists for a given client, topic or partition. A single tick refreshes at most 20 partitions, round-robining the remainder across the following ticks so the listener thread is never stalled for long, and nothing runs during shutdown or quieting. Refreshed data is evicted when partitions resume or get revoked (with a 10x interval expiry as a safety net); errors never propagate but are instrumented via `error.occurred` (type `paused_lags.refresher.error`) and retried with an exponentially backed off delay (up to 8x the interval). Requires `statistics.interval.ms` to be enabled for the overlay to be visible (Pro).
