...
- [Fix] Stop the `JobsQueue` per-group semaphore `Queue` from accumulating unbounded, unconsumed `#tick` signals. The semaphore is only ever used as a level-triggered wakeup, but `#wait` pops at most one entry per loop iteration and, for pure non-blocking/LRJ jobs, never enters its loop at all (`#wait?` short-circuits to `false`), so every `#complete` and every non-blocking job start (`Worker`) pushed a signal that was never drained - unbounded heap growth proportional to the number of non-blocking jobs processed, over the lifetime of the process. `#wait` now drains any stale signals before each `#wait?` check, regardless of whether it ends up blocking. The drain lives in `#wait`, not `#tick`, because `#tick` is invoked concurrently by multiple worker threads (draining there would race), while `#wait` is only ever invoked by the single listener thread owning that group, so it can drain without any extra synchronization. `Pro::Processing::JobsQueue#wait` has its own loop for the async-locking case that does not call `super`, so it drains the same way at the same two points.
