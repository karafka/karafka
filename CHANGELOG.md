...
## 2.6.1 (Unreleased)
- **[Breaking]** Remove the routing `config(...)` topic DSL (the declaratives bridge). Declarative topic structure (partitions, replication factor, Kafka-level config) is now defined exclusively via the standalone declaratives DSL (`Karafka::App.declaratives.draw { topic(name) { partitions ...; replication_factor ...; config(...) } }`), fully independent from routing. Move any `config(...)` calls out of your `routes.draw` topic blocks into a `declaratives.draw` block. Declarative topics are now validated at declaration time inside `declaratives.draw`, and when `strict_declarative_topics` is enabled a routed topic counts as managed only when it has an active declaration in the declaratives repository.
- **[Breaking]** [Pro] Stop declaring the recurring tasks and scheduled messages broker topics as a side effect of routing. Enabling the feature in routing (`recurring_tasks(true)` / `scheduled_messages(name)`) no longer auto-registers the feature's declarative topics; instead declare them explicitly in your declaratives block via the matching `recurring_tasks(true)` / `scheduled_messages(name)` calls (e.g. `Karafka::App.declaratives.draw { recurring_tasks(true) }`). The feature still ships the required functional topic configuration (compaction/retention and, for scheduled messages, the matched partition count), but no longer hardcodes the replication factor - it now falls back to `declaratives.defaults` (or an explicit per-topic override), so production replication is no longer silently set to 1. If you rely on `karafka topics` commands to manage these topics, add the declaratives call; runtime consumption is unaffected.
