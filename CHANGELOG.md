...
- [Fix] Filtering `FiltersApplier#timeout` took the minimum across all applied filters, so when the resolved action was `:pause`, a coexisting filter that resolved to `:seek` (and reports a `0` timeout) collapsed the pause to `0`. The partition then paused with timeout `0`, expired immediately and busy-spun instead of backing off. The pause timeout is now derived only from the filters that actually want to pause (their maximum), so a seek filter's `0` no longer defeats the backoff (Pro).
