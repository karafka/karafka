...
## 2.6.0 (Unreleased)
- **[Feature]** Allow `Karafka::Admin` (and its subsystems like `Admin::ConsumerGroups` and `Admin::Topics`) to operate on a borrowed client: `Karafka::Admin.new(borrowed_client: client)`. Routing is capability based: when the borrowed client is an rdkafka admin instance, admin-based operations (`with_admin` and everything built on it, e.g. `cluster_info`) run on it; any other borrowed client is used by consumer-based operations (`with_consumer` and everything built on it, e.g. `read_lags_with_offsets`). Either way the borrowed instance replaces the short-lived dedicated one, saving a full client bootstrap per call. The borrowed client lifecycle stays fully with its owner: it is never configured, started, unsubscribed or closed by the admin APIs. Low-level internal API: operations run within the borrowed client identity (including its `group.id`) and the caller is responsible for invoking only operations that are safe on a live client (e.g. lag/offset reads for its own group).
