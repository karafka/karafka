...
- [Enhancement] Add `Pro::Processing::ConsumerGroups::Filters::Actions` as the single source of truth for filter post-execution actions: an `ALL` constant (`%i[skip pause seek]`) plus `#skip?`, `#pause?` and `#seek?` predicate helpers built on `#action`. Mixed into both `Filters::Base` and the aggregating `Coordinators::FiltersApplier`, so both expose the same predicate API and all internal call sites use `filter.pause?`/`filter.skip?` instead of comparing the `#action` symbol directly (Pro).
