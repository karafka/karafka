...
- [Fix] Building batch metadata crashed with an `ArgumentError` when the last message in a batch carried no broker timestamp (rdkafka exposes a missing timestamp - `NoTimestampType` - as `nil`): `local_created_at` compared `nil` with `Time.now`. It now falls back to the current time, matching the no-messages path.
