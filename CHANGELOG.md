...
## 2.6.0 (Unreleased)
- **[Feature]** Add `Kubernetes::ReadinessListener` for serving a Kubernetes readiness probe on its own HTTP port. It reports healthy once every active subscription group has polled at least once, and reports not-ready again once the process is shutting down or quieting (`Karafka::App.done?`), so Kubernetes can remove a draining pod from the Service endpoints before it exits. Point a `startupProbe`/`readinessProbe` at it (and a `livenessProbe` at `LivenessListener`). The "all groups have polled" condition latches once met; if the active subscription group set cannot be determined, it reports healthy after any one group has polled.
