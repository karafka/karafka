...
## 2.6.2 (Unreleased)
- **[Feature]** Add an opt-in Ractor-based parallel deserialization engine (requires Ruby 4.0+). Enable it globally with `config.deserializing.parallel.active = true` (`concurrency` and `min_payloads` tune the worker pool size and the minimum batch size worth parallelizing) and per topic with `deserializing(parallel: true)`. A batch below `min_payloads` or with the feature disabled falls back to inline deserialization, so behavior and results are unchanged either way - only the deserialization work for large batches moves off the worker thread and onto a persistent pool of Ractor workers.
- **[Breaking]** Move the default deserializers from `Karafka::Deserializers::{Payload,Key,Headers}` to `Karafka::Deserializing::Deserializers::{Payload,Key,Headers}`, alongside the new parallel deserialization engine. `Karafka::Deserializers` is kept as a backwards compatible alias to the new namespace, so existing references keep working.
