...
- [Fix] Recurring tasks dispatched the schedule state snapshot (which persists each task's `previous_time` after it runs) asynchronously, so a crash after a task executed but before the snapshot was delivered would lose the `previous_time` update and re-run the task on the next replay (double execution). The schedule snapshot is now dispatched synchronously, so it is durably stored before the executor proceeds (Pro).
