...
- **[Feature]** Allow `Karafka::Admin` (and subsystems like `Admin::ConsumerGroups`, `Admin::Topics`) to operate on an external client via `Karafka::Admin.new(external_client: client)`. Accepts a raw rdkafka client, a `Karafka::Connection::Proxy`, or a running `Karafka::Connection::Client` (now exposing `#wrapped_kafka`), so a consumer can read its own lags through its own connection. Reuses the caller's client (no per-call bootstrap); its lifecycle stays with its owner.
