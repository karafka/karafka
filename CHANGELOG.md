...
- [Enhancement] Add `stability_ttl` to `Kubernetes::LivenessListener` and `Pro::Swarm::LivenessListener` to detect consumers frozen in a single non-`"steady"` librdkafka `cgrp.join_state`. The librdkafka `join_state` field tracks the consumer group join protocol: `"steady"` means the consumer has successfully joined the group and is operating normally; any other value (`wait-join`, `wait-assn`, `wait-sync`, `wait-unassign-to-complete`, etc.) means a rebalance or initial join is in progress. Brief non-steady periods are expected during normal rebalances - the `stability_ttl` only triggers when a group stays in the same tracked non-steady state continuously for longer than the threshold, which indicates the consumer is frozen there (e.g. due to broker-side bugs like KAFKA-19862 where the broker hangs in `CompletingRebalance` and never responds to the consumer's JoinGroup/SyncGroup requests). Consumers cycling rapidly between non-steady states are not flagged because each transition resets the timer; cycling indicates the join protocol is still progressing. The pre-join `init` and the leader's mid-join `wait-metadata` states are not tracked and additionally clear any prior tracking, so a consumer transitioning through them does not leave a stale timer that could trip later. Known limitations of this scope: a consumer genuinely frozen in `wait-metadata` (e.g. leader stuck fetching cluster metadata for assignment), one oscillating between `wait-join` and `wait-metadata`, or one cycling across multiple non-steady states will not be caught here - those failure modes are covered by `polling_ttl` instead. When the threshold is exceeded, `stability_ttl_exceeded` is set to `true` in the `Kubernetes::LivenessListener` HTTP response body (HTTP 500) and status code `4` is reported by `Pro::Swarm::LivenessListener`. Recovery is automatic: as soon as `join_state` returns to `"steady"`, the flag clears and the probe returns healthy. **Requires `statistics.interval.ms` to be set** in the Kafka client config - without it `statistics.emitted` never fires and this check has no effect (no false positives). Defaults to `max.poll.interval.ms * 2`, derived lazily on first health evaluation as the maximum `max.poll.interval.ms` across all active subscription groups (whose configs already include librdkafka's 5-minute default via `DefaultsInjector` when the user has not set it), so per subscription group overrides set via the routing DSL are respected and the threshold automatically scales with the largest configured poll interval in the process. When no subscription groups are available, the root `Karafka.config.kafka` value is used instead.
