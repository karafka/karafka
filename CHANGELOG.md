...
- [Enhancement] Introduce the share group (KIP-932 / Queues for Kafka) routing layer: a `share_group` routing block (sibling of `consumer_group`), `#group_type`/`#consumer_group?`/`#share_group?` introspection, `App.share_groups` plus chainable `App.routes.consumer_groups`/`App.routes.share_groups` type-filtered views, and `--include_share_groups`/`--exclude_share_groups` CLI filters (also in swarm mode). Describing share groups is supported; running one raises `Karafka::Errors::ShareGroupsNotImplementedError` until the runtime lands. Routing is reorganized internally into per-mode `Routing::{ConsumerGroups,ShareGroups}` namespaces, with the flat `Routing::ConsumerGroup`/`Topic` constants kept as aliases (retired in 3.0) and the legacy flat custom-feature layout (`module Topic` / `Contracts::Topic`) still supported. Group names must be unique across group types (single Kafka group-id namespace), regexp-style topic definitions (a `Regexp` or a librdkafka `"^"` pattern string) are rejected in share groups with a clear error since share consumers do not support pattern subscriptions, `--share_groups` filters are validated against the routing, swarm fails fast pre-fork on active share groups, and consumer-group-only tooling (Pro swarm routing validation, CLI topic filters, parallel segments, admin lag queries) correctly ignores share groups. No behavior change for consumer-group routing.
