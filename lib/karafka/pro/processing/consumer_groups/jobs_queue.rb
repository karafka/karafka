  ...
  module Pro
    module Processing
      module ConsumerGroups
        ...
        #
        # Aside from the OSS queue capabilities it allows for jobless locking for advanced
        # schedulers
        class JobsQueue < Karafka::Processing::ConsumerGroups::JobsQueue
          ...
          # Method that allows us to lock queue on a given subscription group without enqueuing a
          ...
          # @see `Karafka::Processing::ConsumerGroups::JobsQueue`
          #
          ...
          #
          # @note This implementation takes into consideration temporary async locks that can
          #   happen. Thanks to the fact that we use the minimum lock time as a timeout, we do not
          #   have to wait a whole ticking period to unlock async locks.
          def wait(group_id)
            ...
            # We do not generalize this flow because this one is more expensive as it has to
            # allocate extra objects. That's why we only use it when locks are actually in use
