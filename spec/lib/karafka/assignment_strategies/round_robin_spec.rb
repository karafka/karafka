...
RSpec.describe Karafka::AssignmentStrategies::RoundRobin do
  describe "#call" do
    ...
    let(:members) do
      Hash[(1..5).map { |i| ["member#{i}", nil] } ]
    end
    let(:partitions) do
      (1..10).map { |i| OpenStruct.new(name: "partition#{i}", topic: "greetings", partition_id: i) }
    end
    ...
    it "is delegated to Kafka::RoundRobinAssignmentStrategy" do
      expect_any_instance_of(Kafka::RoundRobinAssignmentStrategy).to receive(:call)
