...
RSpec.describe Karafka::Routing::ConsumerGroup do
  subject(:consumer_group) { described_class.new(id) }
  ...
  (
    Karafka::AttributesMap.consumer_group - %i[batch_consuming topic_mapper]
  ).each do |attribute|
  ...
  %i[batch_consuming topic_mapper].each do |attribute|
    context attribute.to_s do
