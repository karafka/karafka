...
RSpec.describe_current do
  ...
  context "when there are no messages" do
    let(:factories) { [->(*) { Karafka::Pro::Processing::ConsumerGroups::Filters::Throttler.new(10, 10) }] }
    let(:messages) { [] }
  ...
  context "when there are messages and filtering happened" do
    let(:factories) { [->(*) { Karafka::Pro::Processing::ConsumerGroups::Filters::Throttler.new(0, 10) }] }
  ...
  context "when there are messages and multiple filters applied" do
    let(:factories) do
      [
        ->(*) { Karafka::Pro::Processing::ConsumerGroups::Filters::Throttler.new(0, 10) },
        ->(*) { Karafka::Pro::Processing::ConsumerGroups::Filters::Throttler.new(1, 0) }
      ]
    end
  ...
  context "when there are messages and multiple filters applied but none pauses" do
    let(:factories) do
      [
        ->(*) { Karafka::Pro::Processing::ConsumerGroups::Filters::Throttler.new(1, 10) },
        ->(*) { Karafka::Pro::Processing::ConsumerGroups::Filters::Throttler.new(1, 0) }
      ]
    end
