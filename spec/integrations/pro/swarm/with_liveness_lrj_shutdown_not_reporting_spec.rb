...
# `while running?` loop and enters `wait_pinging`. Currently `wait_pinging` does not fire any
# instrumentation events (no `connection.listener.fetch_loop`, no `client.events_poll`), so the
# Pro liveness listener's `report_status` is never called and the node goes silent.
#
...
# Prepend a tracker to detect whether report_status is called during wait_pinging.
# During normal operation, report_status is triggered by fetch_loop and events_poll events.
# After shutdown begins, the listener enters wait_pinging where these events stop firing,
# so report_status is never called and the node goes silent.
module WaitPingingReportTracker
  def report_status
    super

    if Karafka::App.done?
      @shutdown_start ||= Process.clock_gettime(Process::CLOCK_MONOTONIC)
      elapsed = Process.clock_gettime(Process::CLOCK_MONOTONIC) - @shutdown_start

      # After a delay we are past the running->quieting transition and into wait_pinging
      if elapsed > (MACOS ? 4.0 : 3.0)
        LIVENESS_W.puts("1")
        LIVENESS_W.flush
      end
    end
...
Karafka::Pro::Swarm::LivenessListener.prepend(WaitPingingReportTracker)
