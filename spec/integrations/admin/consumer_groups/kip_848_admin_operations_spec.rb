...
lags = Karafka::Admin.read_lags_with_offsets({ DT.group => [DT.topic] })

assert lags.key?(DT.group), "Should be able to read lags for KIP-848 consumer group"
assert lags[DT.group].key?(DT.topic), "Should have topic info in lags"
